...
export const Link = styled.p<HeaderTextLinkProps>`
  color: white;
  text-decoration: none;
  margin-right: 30px;
...
export const SearchInput = styled.input<HeaderSearchInputProps>`
  ...
  height: 30px;
  font-size: 14px;
  margin-left: ${({ active }): string => (active ? '10px' : '0')};
  padding: ${({ active }): string => (active ? '0 10px' : '0')};
  opacity: ${({ active }): string => (active ? '1' : '0')};
  width: ${({ active }): string => (active ? '200px' : '0px')};
`
